...
# ╔═╡ 39ee9c51-4cb7-45e1-ad46-ce4d7ef23420
begin
	using Zygote: Zygote
	using Flux:Flux
...
# ╔═╡ e4037b9e-6a3f-4e96-8579-b563f556e5a8
using Optim
...
evals = sample_points(mnistSimpleCNN7, model_origin=reference_pt)
...
# ╔═╡ f0269508-8792-434a-acf0-ef4cf18a94ba
rand_directions
...
plot(distances_from_ref, seriestype=:hist, label="Distances from Reference Point")
...
begin
	local plt = plot(
		...
		map(x->x.loss, evals),
		seriestype=:scatter,
		title="Stationarity is plausible",
		ylabel="Loss",
...
function negLogLikelihood(params, z=map(e->e.loss, evals[1:npoints]))
	mean = params[1]
	cov = C(params[2:end]...)
	return log(LinearAlgebra.det(cov)) + LinearAlgebra.dot(z, cov\z)
end
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "116d9f89178f9da7f712ef3d4c09ef8b6d3f51db"
